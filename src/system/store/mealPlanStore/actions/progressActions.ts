...
export const createProgressActions = (
  ...
  startProgressSimulation: () => {
    ...
    const updateProgress = () => {
      ...
      // Check if backend has provided recent updates (within last 2 seconds)
      if (now - currentState.lastBackendProgressUpdate < 2000 && currentState.lastBackendProgressUpdate > 0) {
      ...
      if (currentStepIndex >= PROGRESS_SIMULATION_STEPS.length) {
        // All steps completed, stay at 95% until backend completes
        set({
